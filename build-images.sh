...
# Reuse existing nodebuilder-2fauth container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-2fauth; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-2fauth -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/2fauth/${app}:${app_version}" \
    "${container}"
# Commit the image
